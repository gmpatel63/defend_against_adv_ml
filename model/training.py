...
def train_model(args, params, paths):
    ...
    training_dataset = create_tf_dataset(paths['training_data'], args, params, training=True)
    validation_dataset = create_tf_dataset(paths['validation_data'], args, params)
    ...
    with strategy.scope():
        if args.model == 'cnn':
            ...
            model = context_aware_model(cnn_dir)
    ...
    predictions_csv_path = Path(args.experiment_dir, f'{args.model}_predictions_csv.csv')
    test_df.to_csv(predictions_csv_path, index=False, columns=['dataset', 'site_name', 'subject_id', 'age', 'predictions'])